...
-----------------------------------------
-- UNION
SELECT NombreComun FROM plantas_medicinales
UNION ALL -- distinct values, ALL = todos
SELECT NombreComun FROM plantas_exoticas

----------------------------------
-- Docker :
-- docker container ls
-- docker exec -it 85f27b37c34f /bin/bash

-- Crear un archivo binario
-- echo "text in file" > test1.txt
--Después de haber creado un archivo dentro del container:
...
SELECT 'Flor con IMAGE', 'Flor con IMAGE', 'Flor con IMAGE', @descripcion,  * FROM OPENROWSET(BULK N'/tmp/test2.txt', SINGLE_BLOB) AS Foto; 
...
-- Insertar los datos binarios en una tabla: 
INSERT INTO dbo.tabla1(ID, data)  
SELECT 1,   * FROM OPENROWSET(BULK N'/tmp/test1.txt', SINGLE_BLOB) AS data; 
